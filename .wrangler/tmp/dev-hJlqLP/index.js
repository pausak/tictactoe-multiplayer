...
// .wrangler/tmp/bundle-CD0fpq/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-CD0fpq/middleware-insertion-facade.js
init_modules_watch_stub();
...
var GameRoom = class {
  ...
  async fetch(request) {
    ...
    if (this.clients.length === 2) {
      for (const client2 of this.clients) {
        client2.ws.send(JSON.stringify({ type: "force_reset", starter: "X" }));
      }
    }
    ws.addEventListener("message", (event) => {
      try {
        const data = JSON.parse(event.data);
        if (data.type === "move" || data.type === "rematch_request" || data.type === "rematch_accept" || data.type === "hide_room_info") {
          for (const client2 of this.clients) {
            if (client2.ws !== ws) {
  ...
  _cleanDuplicates() {
    const cleaned = [];
    const symbolsSeen = /* @__PURE__ */ new Set();
    for (const client of this.clients) {
      if (client.ws.readyState === WebSocket.OPEN && !symbolsSeen.has(client.symbol)) {
        cleaned.push(client);
